...
def get_current_user_cart_items(Database_name, customer_id, item_id):
    try:
        return Database_name.query.filter_by(
            customer_id=customer_id, product_id=item_id
        ).first()
...
@cache.cached(
    timeout=60, unless=lambda: current_user.is_authenticated, query_string=True
)
def get_all_items():
    ...
    total = db.session.execute(
        db.select(func.count()).select_from(ProductInformation)
    ).scalar()
    return render_template(
        "index.html", all_items=items, page=page, per_page=per_page, total=total
    )
...
def show_item(item_id):
    ...
    return render_template(
        ...
        in_wishlist_check=in_wishlist_check,
    )
...
def search_product():
    ...
    if form.validate_on_submit():
        ...
        return render_template(
            "search.html", form=form, results=results, query=user_query
        )
...
def add_to_cart(item_id):
    ...
    existing_item = get_current_user_cart_items(
        CartDatabase, current_user.get_id(), item_id=item_id
    )
...
@login_required
def wishlist_page():
    user_wishlist_items = get_all_current_user_database_item(
        WishList, current_user.get_id()
    )
...
def remove_from_cart(item_id):
    requested_item_for_cart = db.get_or_404(ProductInformation, item_id)
    item_to_remove = get_current_user_cart_items(
        CartDatabase, current_user.get_id(), item_id=requested_item_for_cart.id
    )
...
def create_checkout_session():
    try:
            ...
            line_items.append(
                {
                    "price_data": {
                        ...
                        "unit_amount": int(
                            cart_item.product.discount_product_price * 100
                        ),
                    },
        ...
        checkout_session = stripe.checkout.Session.create(
            ...
            mode="payment",
            success_url=url_for("main.success_page", _external=True)
...
def clear_cart_items():
    user_cart_items = CartDatabase.query.filter_by(
        customer_id=current_user.get_id()
    ).all()
...
def success_page():
    ...
    try:
        ...
        if checkout_session.payment_status == "paid":
            ...
            total_price = sum(
                (c.product.discount_product_price * c.quantity) for c in user_cart_items
            )
            user_order_items = [
                OrderItem(
                    quantity=c.quantity,
                    price=c.product.discount_product_price,
...
def process_orders():
    ...
    return render_template(
        ...
        user_shipping_details=user_shipping_details,
    )
